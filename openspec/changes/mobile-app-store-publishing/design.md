...
**Amended during implementation (2026-07-12):**
1. *`/api/public/internal-events` was REMOVED, not hardened.* Its supposed consumer (`fetchInternalEvents` in `EventSearchExport.jsx`) turned out to be dead code — never invoked, and reading `event.externalData`, a field deleted in the data-model consolidation. ESLint already flagged it as unused. With zero live consumers, deleting the endpoint beats projecting it.
2. *The query is on `calendarData.*`, not top-level fields.* `buildEventFields` (`utils/eventFieldBuilder.js`) writes title/dates/locations/categories into `calendarData`; `topLevelFields` does not carry them, and `getUnifiedEvents` — the query behind the real calendar — reads `calendarData.startDateTime`. The `calendarData`-removal refactor is still in progress, so `calendarData` is authoritative today. Returning docs in that shape also means `transformEventToFlatStructure` consumes them unchanged.
3. *The endpoint returns occurrences and expands recurring series server-side.* `MobileAgenda` does no recurrence expansion (only `Calendar.jsx` does, client-side), so without server expansion every weekly service would appear exactly once on the public calendar. Expansion reuses `expandRecurringOccurrencesInWindow`, honors `recurrence.exclusions`, and suppresses any date owned by an exception/addition child document (the child is authoritative for its date; a deleted child means a cancelled occurrence). The spec's original "exclude exception/addition children" bullet was a list-endpoint rule and is wrong here — see the updated `public-events-access` spec.
