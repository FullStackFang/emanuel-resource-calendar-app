...
// VALID_ROLES imported from authUtils (single source of truth in permissionUtils.js)
...
/**
 * Build effective rooms/times for an event that has a pending edit request.
 * Merges proposedChanges (delta) with the event's calendarData to get the
 * effective state the event will have once the edit is approved.
 * @param {Object} event - Event document with pendingEditRequest
 * @returns {Object} Merged rooms/times from proposed changes + current calendarData
 */
...
// All per-occurrence Graph syncing is now handled by syncExceptionDocumentsToGraph.
// Exception/addition documents are the sole storage for per-occurrence data.
...
async function checkRoomConflicts(reservation, excludeId = null) {
  try {
    ...
    // Pending recurring series produce soft conflicts (informational, not blocking)
    const pendingRecurringConflicts = [];
        ...
        for (const occ of occurrences) {
          ...
          if (occ.startDateTime < effectiveEndStr && occ.endDateTime > effectiveStartStr) {
            ...
            if (master.status === 'pending') {
              pendingRecurringConflicts.push(conflictEntry);
            } else {
    ...
    for (const peEvent of pendingEditEvents) {
      ...
      // Check time overlap using effective start/end from the pending edit
      const peStart = new Date(effective.startDateTime);
      const peEnd = new Date(effective.endDateTime);
      const peSetup = effective.setupTimeMinutes || 0;
      const peTeardown = effective.teardownTimeMinutes || 0;
      const peEffectiveStart = new Date(peStart.getTime() - (peSetup * 60 * 1000));
      const peEffectiveEnd = new Date(peEnd.getTime() + (peTeardown * 60 * 1000));

      // Time overlap check (same 3-case logic as published events)
      const timeOverlap =
      ...
      pendingEditConflicts.push({
        ...
        eventTitle: effective.eventTitle || peEvent.eventTitle || cd.eventTitle,
    ...
    // Return detailed conflict information (published + pending edit conflicts)
    // Fall back to calendarData fields since published/reservation events store data there
    const publishedConflictResults = actualConflicts.map(conflict => ({
    ...
    const pendingRecurringResults = pendingRecurringConflicts.map(conflict => ({
      id: conflict._id.toString(),
      eventTitle: conflict.eventTitle || conflict.calendarData?.eventTitle,
      startDateTime: conflict._occurrenceStartDateTime || conflict.calendarData?.startDateTime,
      endDateTime: conflict._occurrenceEndDateTime || conflict.calendarData?.endDateTime,
      rooms: conflict.calendarData?.locations,
      status: conflict.status,
      isPendingRecurring: true,
    }));

    const allSoftConflicts = [...pendingEditConflicts, ...pendingRecurringResults];
    return {
      hardConflicts: publishedConflictResults,
      softConflicts: allSoftConflicts,
      allConflicts: [...publishedConflictResults, ...allSoftConflicts]
    };
  } catch (error) {
...
app.get('/api/graph/events', verifyToken, async (req, res) => {
  try {
    ...
    // Get user from database to determine actual role (respects X-Simulated-Role)
...
async function upsertUnifiedEvent(userId, calendarId, graphEvent, internalData = {}, sourceCalendars = []) {
  try {
    ...
    // Per-occurrence overrides are now stored as exception documents (not on master)
    ...
    // Preserve original user-defined recurrence pattern (top-level is authoritative)
...
function projectEventForSSE(event) {
  ...
  const projected = {
    ...
    calendarData: {
      ...
      assignedRabbi: Array.isArray(cd.assignedRabbi) ? cd.assignedRabbi : (cd.assignedRabbi?.userId ? [cd.assignedRabbi] : []),
      assignedCantor: Array.isArray(cd.assignedCantor) ? cd.assignedCantor : (cd.assignedCantor?.userId ? [cd.assignedCantor] : []),
    },
...
function broadcastEventChange({ eventId, action, actorEmail, requesterEmail, event, oldStatus, newStatus }) {
  try {
    const VIEW_MAP = {
      ...
      'edit-withdrawn':          ['approval-queue', 'my-reservations'],
...
const EVENT_LIST_PROJECTION = {
  ...
  pendingCancellationRequest: 1, statusHistory: 1, _version: 1,
...
app.post('/api/events/load', verifyToken, async (req, res) => {
  try {
    ...
    // Get user's effective role for pending event filtering (respects X-Simulated-Role)
...
app.get('/api/events/list', verifyToken, async (req, res) => {
  try {
    ...
    // Look up user for role checks (respects X-Simulated-Role for admin testing)
    const user = await getCachedUser(userId);
    const effectiveRole = resolveEffectiveRole(req, user, userEmail);
    const canViewAll = ROLE_HIERARCHY[effectiveRole] >= ROLE_HIERARCHY['approver'];
...
app.get('/api/rooms/availability', async (req, res) => {
  try {
    ...
    const availability = rooms.map(room => {
      ...
      // Process pending edit events for this room
      const detailedPendingEdits = pendingEditEvents
        .filter(peEvent => {
          const effective = buildEffectiveEditData(peEvent);
          const effectiveLocations = (effective.locations || []).map(id => id.toString());
          return effectiveLocations.includes(roomIdString);
        })
        .map(peEvent => {
          const effective = buildEffectiveEditData(peEvent);
          const cd = peEvent.calendarData || {};
          const effectiveStartDT = effective.startDateTime;
          const effectiveEndDT = effective.endDateTime;
          ...
          return {
            ...
            requesterName: peEvent.pendingEditRequest?.requestedBy?.name || '',
            requesterEmail: peEvent.pendingEditRequest?.requestedBy?.email || '',
            status: 'pending-edit',
            originalStart: effectiveStartDT,
  ...
  try {
    ...
    if (canAutoPublish) {
      ...
      if (draftRecurrence?.pattern && draftRecurrence?.range) {
        // Fix 4: Read timezone from draft data instead of hardcoding
        const draftTimezone = draft.graphData?.start?.timeZone || 'America/New_York';
        const graphRecurrence = buildGraphRecurrence(draftRecurrence, draftTimezone);
        if (graphRecurrence) {
      ...
      await withCosmosRetry(() => unifiedEventsCollection.updateOne(
        { _id: new ObjectId(draftId) },
        {
          ...
          $push: {
            statusHistory: buildStatusHistoryEntry('published', userId, userEmail, 'Auto-published on creation')
          }
        }
      ));
      ...
      await withCosmosRetry(() => unifiedEventsCollection.updateOne(
        { _id: new ObjectId(draftId) },
        {
          ...
          $unset: { draftCreatedAt: "" },
          $push: {
            statusHistory: buildStatusHistoryEntry('pending', userId, userEmail, 'Submitted for review')
          }
        }
      ));
  ...
  try {
    ...
    await unifiedEventsCollection.updateOne(
      { _id: new ObjectId(draftId) },
      {
        ...
        $push: {
          statusHistory: buildStatusHistoryEntry('deleted', userId, userEmail, 'Draft deleted by owner')
        }
      }
    );
...
app.put('/api/room-reservations/:id/restore', verifyToken, async (req, res) => {
  try {
    ...
    const updateOp = {
      ...
      $push: {
        statusHistory: buildStatusHistoryEntry(previousStatus, userId, userEmail, `Restored from ${currentStatus} by owner`)
      }
    };
    ...
    if (hadGraphEvent) {
      ...
      if (calendarOwner) {
        try {
          ...
          const restoreRecurrence = reservation.recurrence;
          ...
          if (restoreRecurrence?.exclusions?.length) {
            try {
              ...
              if (exclusionResults.cancelledOccurrences.length) {
                await unifiedEventsCollection.updateOne(
                  { _id: new ObjectId(reservationId) },
                  { $set: { 'graphData.cancelledOccurrences': exclusionResults.cancelledOccurrences } }
                );
              }
            } catch (syncError) {
              logger.warn('Failed to sync recurrence exclusions to Graph on owner restore:', syncError.message);
            }
          }

          // Sync exception/addition documents to Graph
          try {
    ...
    try {
      resultEvent = await conditionalUpdate(
        ...
        { _id: new ObjectId(eventId) },
        {
          ...
          $push: {
            statusHistory: buildStatusHistoryEntry(previousStatus, userId, userEmail, 'Restored by admin')
          }
        },
        {
          expectedVersion: _version || null,
    ...
    if (hadGraphEvent) {
      ...
      if (calendarOwner) {
        try {
          ...
          const adminRestoreRecurrence = event.recurrence;
          if (adminRestoreRecurrence?.pattern && adminRestoreRecurrence?.range) {
            const graphRecurrence = buildGraphRecurrence(adminRestoreRecurrence, eventTimezone || 'America/New_York');
            if (graphRecurrence) {
              graphEventData.recurrence = graphRecurrence;
              // Fix 3: Align start/end dates with recurrence range.startDate
              const rangeStart = graphRecurrence.range.startDate;
          ...
          if (adminRestoreRecurrence?.exclusions?.length) {
            try {
              ...
              logger.warn('Failed to sync recurrence exclusions to Graph on admin restore:', syncError.message);
            }
          }
...
app.put('/api/room-reservations/:id/resubmit', verifyToken, async (req, res) => {
  try {
    ...
    const updateDoc = {
      ...
      $push: {
        statusHistory: buildStatusHistoryEntry('pending', userId, userEmail, 'Resubmitted after rejection')
      }
    };
...
app.put('/api/admin/events/:id/publish', verifyToken, async (req, res) => {
  try {
    ...
    const publishUpdate = {
      ...
      $push: {
        statusHistory: buildStatusHistoryEntry('published', userId, userEmail, notes ? `Published: ${notes}` : 'Published by admin')
      }
    };

    // Set eventType to seriesMaster when publishing a recurring event
    const publishRecurrenceCheck = event.recurrence;
      ...
      try {
        ...
        if (publishRecurrence?.exclusions?.length) {
          try {
            ...
            logger.warn('Failed to sync recurrence exclusions to Graph on publish:', syncError.message);
          }
        }
    ...
    try {
      rejectedEvent = await conditionalUpdate(
        ...
        { _id: new ObjectId(id) },
        {
          ...
          $push: {
            statusHistory: buildStatusHistoryEntry('rejected', userId, userEmail, reason)
          }
        },
        {
          expectedVersion: _version || null,
...
// ============================================================================
// Each edit request is a document in templeEvents__EditRequests with its own
// lifecycle (pending | approved | rejected | withdrawn | superseded), OCC
// _version, statusHistory, and audit trail. One active request per
// (eventId, editScope, occurrenceDate) tuple — series-level and per-occurrence
// edits are independent slots; different occurrenceDate values are independent.
// ============================================================================
...
app.get('/api/events/:id', verifyToken, async (req, res) => {
  try {
    const projection = {
      ...
      pendingCancellationRequest: 1, draftCreatedAt: 1, lastDraftSaved: 1,
...
app.post('/api/events/:id/request-cancellation', verifyToken, async (req, res) => {
  try {
    ...
    // it for ANY pending request on this event regardless of scope, since a
    // pending series-level OR per-occurrence edit should still block a
    // cancellation submission.
    const conflictingPendingEdit = await editRequestsCollection.findOne(
      { eventId: event.eventId, status: 'pending' },
      { projection: { _id: 1 } }
    );
    if (conflictingPendingEdit) {
      return res.status(400).json({
    ...
    try {
      deletedEvent = await conditionalUpdate(
        ...
        { _id: new ObjectId(eventId) },
        {
          ...
          $push: {
            statusHistory: buildStatusHistoryEntry('deleted', userId, userEmail, cancellationReason)
          }
        },
        {
          expectedVersion: _version || null,
...
app.put('/api/admin/events/:id', verifyToken, async (req, res) => {
  try {
    ...
    const {
      ...
      pendingEditRequest: _pendingEditRequest, // Stripped: edit requests live in templeEvents__EditRequests, never in event docs
    ...
    // Sync full Graph API response back to graphData (ONLY when Graph API was actually called)
    // IMPORTANT: This must run AFTER all location/offsite processing to catch all graphData.* paths
    if (graphSyncResult) {
      // Merge full Graph API response into graphData (matches audit-update pattern)
      const updatedGraphData = { ...(event.graphData || {}), ...graphSyncResult };
      updateOperations['graphData'] = updatedGraphData;
      // Remove any nested graphData.* paths to avoid MongoDB ConflictingUpdateOperators
      for (const key of Object.keys(updateOperations)) {
        if (key.startsWith('graphData.')) delete updateOperations[key];
      }
      logger.info('Syncing full Graph API response to MongoDB graphData:', {
        returnedId: graphSyncResult.id,
        returnedSubject: graphSyncResult.subject,
        returnedLocation: graphSyncResult.location
      });
    ...
    // When an approver directly saves edits to a published event that has a
    // pending edit request, prune any fields the approver just addressed from
    // the pending request's proposedChanges so a subsequent approval doesn't
    // re-apply (and revert) the approver's intent. Edit requests now live in
    // the templeEvents__EditRequests collection, so the prune happens via a
    // separate write on that doc rather than a nested-field $set on the event.
    if (event.status === 'published') {
      try {
        const pendingRequests = await editRequestsCollection.find({
          eventId: event.eventId,
          status: 'pending',
        }).toArray();
        ...
        const locationFields = new Set(['locations', 'requestedRooms', 'locationDisplayNames']);

        for (const pending of pendingRequests) {
          const currentProposed = { ...(pending.proposedChanges || {}) };
          ...
          for (const field of Object.keys(currentProposed)) {
            ...
            if (approverTouched || (locationFields.has(field) && approverModifiedLocations)) {
              delete currentProposed[field];
            }
          }
          if (Object.keys(currentProposed).length < originalCount) {
            await editRequestsCollection.updateOne(
              { _id: pending._id },
              {
                $set: { proposedChanges: currentProposed, lastModifiedDateTime: new Date(), lastModifiedBy: userEmail },
                $inc: { _version: 1 },
              }
            );
            logger.info('Pruned proposedChanges after approver edit on published event:', {
              eventId: id,
              editRequestId: pending.editRequestId,
        ...
        logger.warn('Failed to prune proposedChanges after approver edit (non-blocking):', mergeErr.message);
      }
    }
...
app.delete('/api/admin/events/:id', verifyToken, async (req, res) => {
  try {
    ...
    // Check permissions (requester+ can delete with scoping rules, respects X-Simulated-Role)
    const user = await getCachedUser(userId);
    const effectiveRole = resolveEffectiveRole(req, user, userEmail);
    const isAdminUser = effectiveRole === 'admin';
      ...
      try {
        deletedExDoc = await conditionalUpdate(
          ...
          { _id: new ObjectId(id), isDeleted: { $ne: true } },
          {
            ...
            $push: {
              statusHistory: buildStatusHistoryEntry('deleted', userId, userEmail, reason?.trim() || `Occurrence ${dateKey} deleted`)
            }
          },
      ...
      try {
        ...
        if (editScope === 'thisEvent' && seriesMasterId && occurrenceDate) {
          ...
          if (additionEntry) {
            ...
            // occurrenceDate is YYYY-MM-DD; build local-time range strings directly
            // to avoid UTC shift from new Date() + toISOString()
            ...
            try {
              ...
              if (instances && instances.length > 0) {
                const targetOccurrence = instances.find(occ => {
                  // Match by date portion only (YYYY-MM-DD)
                  return occ.start?.dateTime?.startsWith(occurrenceDate);
                });
    ...
    if (editScope === 'thisEvent') {
      ...
      const updateOps = {
        $addToSet: { [recurrencePath + '.exclusions']: dateKey },
        $push: {
          statusHistory: buildStatusHistoryEntry(event.status, userId, userEmail, `Occurrence ${dateKey} excluded (deleted)`)
        },
        $set: {
          lastModifiedDateTime: new Date(),
          lastModifiedBy: userId,
      ...
      if (recurrence.additions && recurrence.additions.includes(dateKey)) {
        await unifiedEventsCollection.updateOne(
          { _id: new ObjectId(id) },
          { $pull: { [recurrencePath + '.additions']: dateKey } }
        );
      }
