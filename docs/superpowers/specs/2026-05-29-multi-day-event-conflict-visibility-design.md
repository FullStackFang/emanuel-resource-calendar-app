...
- **2026-05-29 — Part 2 pivoted to live verdict (B) after a real-app test.** Scope-honest note (A) didn't satisfy the need — a multi-day draft with a conflict on a MIDDLE day stayed silent in the assistant until the 409. Added read-only `GET /api/rooms/conflict-check` (dry-run of `checkRoomConflicts`) + wired form/assistant to show the real full-span verdict (room · day — title / all-clear / checking). Preview === save (same function). Also fixed the draft-submit 409 message (5th site). 6 assistant tests + build green. **Backend endpoint lacks an integration test (testApp legacy); needs live verification** with the multi-day + middle-day-conflict scenario. Also moved the verdict banner to the TOP of the assistant (under the header) so it's impossible to miss.
- **2026-05-29 — Part 4 implemented (Warn + Save Anyway for staff).** Backend: widened the `forceUpdate` gate on `PUT /api/admin/events/:id` from `effectiveRole !== 'admin'` to `!hasApproverAccess` (entry already requires approver, so requesters/guests can't reach it; this also aligned api-server with testApp, whose copy never had the admin-only gate). Frontend: hard-conflict 409 with `canForce` now pops a `forceConflictConfirmation` → new `hard_conflict` ConflictDialog variant ("Save Anyway") whose `retryFn` resends with the force flag (mirrors the soft-conflict retry). **Scope: admin-save path only**; publish (`forcePublish`, `:21005`) and draft-submit stay as-is. 5 new ConflictDialog tests + build green. **No backend integration test** (testApp diverged — it already allowed approver force, so a test there wouldn't validate the api-server gate); needs live verification by editing a PUBLISHED event as an approver into a conflict.
