...
//
// These shared mailboxes are delegated-access resources. The selected calendar is a
// system-wide setting controlled by the admin's 'Default Calendar' configuration, NOT
// a per-user preference. There is intentionally no saved-user-preference priority.
//
// Order (top-most match wins):
//   1. admin-default      — admin-configured default from allowedConfig.defaultCalendar
//   2. app-config-default — APP_CONFIG.DEFAULT_DISPLAY_CALENDAR
//   3. first-allowed      — first calendar whose owner is in allowedDisplayCalendars
//   4. none               — no allowed calendar matched; caller should hard-fail
...
export function selectDefaultCalendar({
  ...
  // 4. No allowed match. The caller MUST hard-fail (toast + leave selection null),
